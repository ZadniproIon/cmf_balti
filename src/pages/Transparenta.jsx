...
const Transparenta = () => {
  ...
  const categoryLabel = (category) => t(`transparenta.categories.${category}`)
  ...
  return (
    <>
      <section className="section-text" aria-labelledby="transparenta-title">
        <h1 id="transparenta-title" className="sr-only">{t('transparenta.title')}</h1>
        <div className="transparenta-filters" role="group" aria-label="Filtre documente">
          <label>
            <span>{t('transparenta.filters.category')}</span>
            <select value={selectedCategory} onChange={(event) => setSelectedCategory(event.target.value)}>
              <option value="all">{t('transparenta.filters.allCategories')}</option>
          ...
          <label>
            <span>{t('transparenta.filters.year')}</span>
            <select value={selectedYear} onChange={(event) => setSelectedYear(event.target.value)}>
              <option value="all">{t('transparenta.filters.allYears')}</option>
        ...
        <ul className="transparenta-results" role="list">
          {filteredDocuments.length === 0 ? (
            <li className="transparenta-empty">{t('transparenta.empty')}</li>
          ) : (
            filteredDocuments.map((doc, index) => (
              <li className="transparenta-result" key={`${doc.category}-${doc.year}-${index}`}>
                <div className="transparenta-result-text">
                  ...
                  <p className="transparenta-result-meta">{categoryLabel(doc.category)} • {doc.year}</p>
                </div>
                <a href={doc.href} target="_blank" rel="noreferrer">
                  <Download className="download-icon" />{t('transparenta.cta')}
                </a>
              </li>
            ))
          )}
